...
with open(f"{BASE_LOCATION}/DataScaling.pkl", "rb") as feature_scaling:
    scaling = pickle.load(feature_scaling)


@app.post(path="/results")
def forecast(data: Features):
    scalingData = scaling.transform([[data.age, data.sex, data.cp, data.trtbps, data.chol, data.fbs,
                                 data.restecg, data.thalachh, data.exng, data.oldpeak, data.slp,
                                 data.caa, data.thall]])
    prediction = model.predict(scalingData)
